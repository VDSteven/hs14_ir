    ...
    */
    ...
    if (model == 0) {
      ...
      for(doc <- tipster.stream) {
        df ++= StopWords.filter(doc.tokens.distinct).map(t => t -> (1+df.getOrElse(t, 0)))
        dcount += 1
      }
      
      val invdf = idf(df.toMap, dcount)
      for(doc <- tipster.stream) {
        alerts.processTerm(doc.name, StopWords.filter(doc.tokens).toList, invdf)
      }
    }
    //Language model alert
      ...
      for(doc <- tipster.stream) {
        ...
        cf ++= StopWords.filter(doc.tokens.distinct).map(t => t -> (cf.getOrElse(t, 0)+tfs.getOrElse(t, 0)))
      }
      
      val smooths = alerts.smooth(cf.toMap, cf.values.sum)
      for(doc <- tipster.stream) {
        alerts.processLanguage(doc.name, StopWords.filter(doc.tokens).toList, smooths)
      }
    }
    ...
    var mapsum = 0.0
    for (i <- (0 to 39)) {
      val rel = new TipsterGroundTruth("qrels").judgements.get((i+51).toString).get.toSet
